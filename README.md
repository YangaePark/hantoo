...
- 기본 최대 동시보유는 1종목입니다. 신규 매수 1건의 예산은 자동매매 시드를 최대 동시보유 수로 나눈 슬롯 금액을 기준으로 제한합니다.
...
- 자동 프로파일(`adaptive_market_regime`)이 켜져 있으면 장중 상태를 `정상/신규진입 차단`으로 판별해 약한 장에서는 신규 매수를 멈춥니다.
- 대시보드는 실시간 진입 상태 배지와 상태 전환 횟수, 손절 후 재진입 차단 횟수, 추정 손실회피 금액을 함께 표시합니다.
- 손절(`live_stop_loss`) 후 동일 종목 재진입은 설정 시간만큼 차단합니다. 추적손절(`live_trailing_stop`) 후에는 그 절반 시간만 차단해 수익 실현 후 재진입 기회를 열어 둡니다. 이 차단은 모든 진입 경로(백테스터 연계 진입 및 직접 진입 모두)에 적용됩니다.

## 트레이드 알고리즘

국내와 해외는 완전히 별도 트레이더로 실행됩니다. 키, 계좌, 자동매매 설정, 리포트 폴더, 보유 포지션, 주문 로그가 서로 분리되므로 한 슬롯을 중지해도 다른 슬롯은 계속 돌 수 있습니다.

### 시장별 전략 한눈에 보기

| 구분 | 국내 | 해외 (NASDAQ) |
| --- | --- | --- |
| 대상 | 변동성 큰 국내 개별주 | NASDAQ 개별주 (ETF/ETN 제외) |
| 자동선별 소스 | 거래대금/거래량급증/등락률/체결강도 | 거래대금/거래량/상승률/거래량급증/체결강도 |
| 기본 관찰 시간 | 10분 | 본장 20분, 프리장 30분 |
| 진입 시간 제한 | 전략 설정 기준 | 장세션(본장/프리장) 기준 |
| 핵심 진입 필터 | 갭상승 + 거래량 + ATR + VWAP + 관찰구간 고가 돌파 | 갭상승 + 거래량 + ATR + VWAP + 관찰구간 고가 돌파 |
| 위험관리 | 손절/익절/추적손절/강제청산 | 본장/프리장 각각 별도 손절/익절/추적손절 |
| 일일 제한 | 일일 진입 횟수 제한 | 일일 진입 횟수 제한 (프리장은 더 보수적) |
...
- 국내 단타 전략
  ...
  - 장이 약하면 신규진입 차단 상태로 전환해 새 매수를 멈추고, 정상 상태에서는 기본 전략 조건을 그대로 적용합니다.

- 해외 NASDAQ 전략
  - NASDAQ 랭킹 기반으로 유동성 높은 종목을 우선 선별하고, 본장과 프리장을 분리해 리스크를 다르게 관리합니다.
  - 프리장은 스프레드와 유동성 리스크가 커서 거래대금/거래량/VWAP 조건을 본장보다 더 엄격하게 적용합니다.
  - 본장도 자동 진입 게이트를 적용해 약한 장에서는 새 매수를 멈추고, 정상 장에서는 기본 조건대로 진입합니다.
...
- 손절 후 동일 종목 재진입: `stop_loss_reentry_block_minutes` 동안 차단
...
9. `자동매매 시작`을 누릅니다.
10. 상단 리포트에서 국내는 `live_trading`, 해외는 `live_trading_overseas`를 선택해 현재 상태, 거래 내역, 횟수, 수익률, 자산 그래프를 확인합니다.
11. 상태줄의 `동시보유: 현재/최대종목`, `분봉`, `봉준비`, `매수대기 사유`, `프로파일`, `진입상태`를 확인합니다. 주문이 없을 때는 `decision_log.jsonl`에서 종목별 탈락 사유와 주문 실패 사유를 볼 수 있습니다. 현재가가 `0`으로 들어오면 KIS 응답의 `rt_cd`, `msg_cd`, `msg1`, `output_keys`도 함께 남습니다.
