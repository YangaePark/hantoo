...
한국투자증권 Open API를 이용해 장중 변동성이 큰 국내 주식, 미국 NASDAQ 주식, 나스닥 급등주, 국내 급등주, 국내 1배 ETF를 자동 선별하고, 단타 전략을 모의 실행하거나 실전 현금 주문으로 실행하는 Python 웹 앱입니다. 별도 웹 프레임워크 없이 Python 표준 라이브러리만 사용합니다.
...
- 최대 12개 종목을 주기적으로 조회하고, 앱 내부에서 시장별 분봉을 만들어 단타 전략을 평가합니다.
- 자동선별 후보는 시장별 주기로 갱신하고, 한번 선별된 종목은 최소 유지 시간 동안 관찰용 분봉 데이터가 쌓이게 합니다.
...
- 웹의 `국내`/`해외`/`나스닥급등주`/`국내급등주`/`국내ETF` 탭은 키, 계좌, 전략 설정, 실행 상태, 리포트를 서로 분리해 각각 시작/중지할 수 있습니다.
- 해외 슬롯은 별도 한국투자 키와 해외계좌를 저장하고, NASDAQ 랭킹 API로 거래대금, 거래량, 상승률, 거래량 급증, 체결강도 후보를 합쳐 자동 선별합니다. 프리장 자동선별은 해외 탭에서 별도로 켤 수 있습니다.
- 나스닥급등주 슬롯은 해외 주문/시세 API를 쓰되 NASDAQ 급등 후보만 1분봉으로 추적하고, 최근 3분 거래대금과 직전 1분봉 고점 돌파를 별도로 확인합니다.
- 국내급등주 슬롯은 국내 순위 API로 등락률, 거래대금, 거래량 급증, 체결강도 후보를 합치고 1분봉 기준 직전 고점 돌파, 최근 3분 거래대금, VI 프록시, 15분 시간손절을 별도로 적용합니다.
- 국내ETF 슬롯은 국내 현금 주문 API를 그대로 쓰되, KODEX/TIGER 등 거래대금이 큰 1배 ETF만 추적하고 레버리지, 인버스, ETN, 저유동성 후보는 제외합니다.
- 자동 프로파일(`adaptive_market_regime`)이 켜져 있으면 장중 톤을 `공격/중립/보수`로 자동 판별해 진입 강도와 리스크 한도를 동적으로 조정합니다.
- 대시보드는 실시간 톤 배지와 톤 전환 횟수, 손절 후 재진입 차단 횟수, 추정 손실회피 금액을 함께 표시합니다.
- 손절(`live_stop_loss`) 후 동일 종목 재진입은 설정 시간(기본 국내 25분, ETF 20분)만큼 차단합니다. 추적손절(`live_trailing_stop`) 후에는 그 절반 시간만 차단해 수익 실현 후 재진입 기회를 열어 둡니다. 이 차단은 모든 진입 경로(백테스터 연계 진입 및 직접 진입 모두)에 적용됩니다.

## 트레이드 알고리즘

국내, 해외, 나스닥급등주, 국내급등주, 국내ETF는 완전히 별도 트레이더로 실행됩니다. 키, 계좌, 자동매매 설정, 리포트 폴더, 보유 포지션, 주문 로그가 서로 분리되므로 한 슬롯을 중지해도 다른 슬롯은 계속 돌 수 있습니다.

### 시장별 전략 한눈에 보기

| 구분 | 국내 | 해외 (NASDAQ) | 나스닥급등주 | 국내급등주 | 국내ETF |
| --- | --- | --- | --- | --- | --- |
| 대상 | 변동성 큰 국내 개별주 | NASDAQ 개별주 (ETF/ETN 제외) | 당일 급등 NASDAQ 개별주 | 당일 급등 국내 개별주 | 국내 1배 ETF (KODEX/TIGER 중심) |
| 자동선별 소스 | 거래대금/거래량급증/등락률/체결강도 | 거래대금/거래량/상승률/거래량급증/체결강도 | 거래대금/거래량/상승률/거래량급증/체결강도 | 등락률/거래대금/거래량급증/체결강도 | 국내 순위 API + ETF 유니버스 |
| 기본 관찰 시간 | 10분 | 본장 20분, 프리장 30분 | 1분봉 4개 이상 | 1분봉 3개 이상 | 5분 |
| 진입 시간 제한 | 전략 설정 기준 | 장세션(본장/프리장) 기준 | 미국 동부 09:35~15:30 | 09:05~14:50 | 09:10~14:30 |
| 핵심 진입 필터 | 갭상승 + 거래량 + ATR + VWAP + 관찰구간 고가 돌파 | 갭상승 + 거래량 + ATR + VWAP + 관찰구간 고가 돌파 | +4~18%, 최근 3분 거래대금, 직전 1분 고점 돌파 | +3~12%, 최근 3분 +2%, 거래대금 10억원, 직전 1분 고점 돌파 | VWAP + 단기 모멘텀 + 거래량 + 지수 ETF 프록시 확인 |
| 위험관리 | 손절/익절/추적손절/강제청산 | 본장/프리장 각각 별도 손절/익절/추적손절 | +4% 익절, -2% 손절, 20분 시간손절, 15:50 청산 | +3% 익절, -2% 손절, 15분 시간손절, 15:10 청산 | 촘촘한 손절/익절 + 부분익절 + 일일 손익 차단 + 연속 손절 차단 |
| 일일 제한 | 일일 진입 횟수 제한 | 일일 진입 횟수 제한 (프리장은 더 보수적) | 1종목 집중, 3회 진입, -2% 손실/3연속 손절 차단 | 1종목 집중, 3회 진입, -1% 손실/3연속 손절 차단 | 일일 목표/손실/연속손절/쿨다운 동시 적용 |
...
- 국내 단타 전략
  ...
  - 장이 강하면 공격 톤으로 진입 조건을 일부 완화하고, 장이 약하면 보수 톤으로 진입 시작 시간을 늦추고 리스크를 줄입니다.

- 해외 NASDAQ 전략
  - NASDAQ 랭킹 기반으로 유동성 높은 종목을 우선 선별하고, 본장과 프리장을 분리해 리스크를 다르게 관리합니다.
  - 프리장은 스프레드와 유동성 리스크가 커서 거래대금/거래량/VWAP 조건을 본장보다 더 엄격하게 적용합니다.
  - 본장도 자동 톤 전환을 적용해 강세장/약세장에 따라 진입 강도와 최대 진입 횟수를 자동 조정합니다.

- 나스닥급등주 전략
  - NASDAQ 상승률 상위만 따라가지 않고 거래대금, 거래량 급증, 체결강도 후보를 함께 확인합니다.
  - 내부 1분봉으로 직전 1분봉 고점 돌파와 최근 3분 달러 거래대금을 확인한 뒤 매수합니다.
  - 기존 해외 주문 API의 지정가 주문을 그대로 사용하고, 20분 시간손절과 당일 강제청산을 적용합니다.

- 국내급등주 전략
  - 등락률 상위만 따라가지 않고 거래대금, 거래량 급증, 체결강도를 함께 확인해 급등 후보를 좁힙니다.
  - 내부 1분봉으로 직전 1분봉 고점 재돌파와 최근 3분 +2% 이상 모멘텀을 확인한 뒤 매수합니다.
  - 신규 매수는 시장가가 아니라 현재가 기준 지정가 주문을 사용하고, VI 프록시 쿨다운과 15분 시간손절로 사고를 줄입니다.
...
3. 후보 종목 현재가를 주기적으로 조회해 내부 봉을 만듭니다. 나스닥급등주와 국내급등주는 1분봉, 나머지는 기본 5분봉입니다.
4. 선별된 종목은 시장별 최소 유지 시간 동안 관찰봉이 쌓이게 합니다.
...
나스닥급등주 자동선별도 NASDAQ으로 한정합니다. `거래대금`, `거래량`, `상승률`, `거래량 급증`, `체결강도` 순위 API를 합치며 ETF/ETN/FUND/TRUST/INDEX/레버리지/인버스 성격 종목은 제외합니다. 1차 후보는 현재가 `5달러` 이상, 전일 대비 `+4% ~ +18%`, 누적 거래대금 본장 `1천만 달러` 이상을 기본으로 보고, 실제 진입은 내부 1분봉에서 최근 3분 거래대금 본장 `200만 달러` 이상과 직전 1분봉 고점 돌파를 다시 확인합니다.

국내급등주 자동선별은 국내 `등락률`, `거래대금`, `거래량 급증`, `체결강도` 순위를 합칩니다. 1차 후보는 전일 대비 `+3% ~ +12%`, 거래대금 `10억원` 이상, 체결강도 `130` 이상, ETF/ETN/SPAC/관리·경고 성격 제외를 통과해야 하며, 실제 진입은 내부 1분봉에서 최근 3분 거래대금 `10억원`과 직전 1분봉 고점 돌파를 다시 확인합니다.
...
해외와 나스닥급등주 프리장은 각 탭에서 `프리장 자동선별 포함`을 켠 경우에만 동작합니다. 프리장은 유동성과 스프레드 위험이 커서 본장보다 더 엄격합니다.
...
나스닥급등주 기본값은 `config/nasdaq_surge_scalp.json` 기준입니다.

- 내부 봉: `1분봉`
- 상승률 조건: 전일 대비 `+4% ~ +18%`
- 최근 거래대금: 본장 최근 `3분 200만 달러` 이상
- 체결강도 조건: 순위 응답 기준 `130` 이상 또는 체결강도 상위 후보
- 진입 확인: 직전 `1분봉 고점` 돌파, VWAP 위, 거래량 증가, VWAP 이격 `7%` 이하
- 신규 진입 중단: 미국 동부 시각 `15:30`
- 하루 최대 진입: `3회`, 기본 동시보유 `1종목`

국내급등주 기본값은 `config/domestic_surge_scalp.json` 기준입니다.

- 내부 봉: `1분봉`
- 상승률 조건: 전일 대비 `+3% ~ +12%`
- 최근 모멘텀: 최근 `3분 +2%` 이상, 최근 `3분 거래대금 10억원` 이상
- 체결강도 조건: 순위 응답 기준 `130` 이상 또는 체결강도 상위 후보
- 진입 확인: 직전 `1분봉 고점` 돌파, VWAP 위, 거래량 증가, VWAP 이격 `4%` 이하
- 신규 진입 중단: `14:50`
- 하루 최대 진입: `3회`, 기본 동시보유 `1종목`

국내ETF 슬롯 기본값은 `config/domestic_etf_scalp.json` 기준입니다.

- 진입 가능 시간: `09:10 ~ 14:30`, 강제청산 기준 `15:15`
...
- 2연속 손절: `45분` 신규 진입 정지
- 3연속 손절: 당일 신규 진입 중단
- 손절 후 동일 종목 재진입: `stop_loss_reentry_block_minutes`(기본 국내 25분, ETF 20분)동안 차단
- 추적손절 후 동일 종목 재진입: 재진입 차단 시간의 절반 동안 차단 (수익 실현 후 재진입 기회 유지)
...
- 나스닥급등주 손절: 진입가 대비 `-2.0%`
- 나스닥급등주 익절: 진입가 대비 `+4.0%`
- 나스닥급등주 시간손절: 진입 후 `20분`
- 나스닥급등주 강제청산: 미국 동부 시각 `15:50`
- 익일로 넘어간 미청산 포지션: 다음 거래 가능 세션에서 강제청산

전략값을 바꾸려면 국내는 `config/volatile_stock_scalp.json`, 해외는 `config/overseas_stock_scalp.json`, 나스닥급등주는 `config/nasdaq_surge_scalp.json`, 국내급등주는 `config/domestic_surge_scalp.json`, 국내ETF는 `config/domestic_etf_scalp.json`을 수정하면 됩니다. 웹에서 바꾸는 계좌, 시드, 자동시작, 프리장 포함, 최대 동시보유 값은 `${SEMIBOT_STATE_ROOT}/config/live*.local.json`에 따로 저장됩니다.
...
1. `국내`, `해외`, `나스닥급등주`, `국내급등주`, `국내ETF` 탭 중 실행할 시장을 선택합니다.
...
5. 해외와 나스닥급등주 탭은 NASDAQ 자동선별만 사용합니다. 주문 거래소는 `NASD`, 시세 거래소는 `NAS`, 통화는 `USD`로 고정됩니다. 프리장을 포함하려면 `프리장 자동선별 포함`을 켭니다.
...
9. `자동매매 시작`을 누릅니다.
10. 상단 리포트에서 국내는 `live_trading`, 해외는 `live_trading_overseas`, 나스닥급등주는 `live_trading_nasdaq_surge`, 국내급등주는 `live_trading_domestic_surge`, 국내ETF는 `live_trading_domestic_etf`를 선택해 현재 상태, 거래 내역, 횟수, 수익률, 자산 그래프를 확인합니다.
11. 상태줄의 `동시보유: 현재/최대종목`, `분봉`, `봉준비`, `매수대기 사유`, `프로파일`, `톤`을 확인합니다. 주문이 없을 때는 `decision_log.jsonl`에서 종목별 탈락 사유와 주문 실패 사유를 볼 수 있습니다. 현재가가 `0`으로 들어오면 KIS 응답의 `rt_cd`, `msg_cd`, `msg1`, `output_keys`도 함께 남습니다.
...
- `${SEMIBOT_STATE_ROOT}/config/kis.nasdaq_surge.local.json`: 나스닥급등주 슬롯용 한국투자 키와 Access Token
- `${SEMIBOT_STATE_ROOT}/config/kis.domestic_surge.local.json`: 국내급등주 슬롯용 한국투자 키와 Access Token
...
- `${SEMIBOT_STATE_ROOT}/config/live.nasdaq_surge.local.json`: 나스닥급등주 자동매매 설정
- `${SEMIBOT_STATE_ROOT}/config/live.domestic_surge.local.json`: 국내급등주 자동매매 설정
...
- `${SEMIBOT_STATE_ROOT}/reports/live_trading_nasdaq_surge/`: 나스닥급등주 슬롯 리포트. 파일 구성은 해외와 같습니다.
- `${SEMIBOT_STATE_ROOT}/reports/live_trading_domestic_surge/`: 국내급등주 슬롯 리포트. 파일 구성은 국내와 같습니다.
